...
class BasicClientActor(BasicClientActorAbs):
    def __init__(self, hex, player, IP_address=None, verbose=True):
    ...
    def handle_get_action(self, state):
        ...
        game = (state[0], state[1:], None)

        return self.hex.action_from_distribution(
            game, self.player.get_distribution(game))
...
if __name__ == '__main__':
    ...
    from hex import Hex
    from generate_model import HexPlayer

    bsa = BasicClientActor(Hex(5), HexPlayer(sys.argv[1]), verbose=True)
